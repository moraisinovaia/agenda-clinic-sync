...
// Helper function to check if doctor is Dr. Marcelo
  const isMedicoMarcelo = (medicoId?: string): boolean => {
    ...
    return !!medicoId && drMarceloIds.includes(medicoId);
  };
  ...
  const validateFormData = (formData: SchedulingFormData) => {
    ...
    // Data de nascimento é obrigatória apenas se não for Dr. Marcelo
    if (!isMedicoMarcelo(formData.medicoId) && !formData.dataNascimento) {
    ...
    // Validar idade do paciente apenas se tiver data de nascimento
