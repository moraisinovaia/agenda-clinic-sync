-- Ajustar comparação de convênio para ignorar espaços extras (TRIM)
...
BEGIN
  ...
  -- Validar convênio com comparação CASE-INSENSITIVE e ignorando espaços extras
  IF v_doctor_record.convenios_aceitos IS NOT NULL AND array_length(v_doctor_record.convenios_aceitos, 1) > 0 THEN
    -- Verificar se o convênio existe no array (case-insensitive + TRIM)
    SELECT EXISTS (
      SELECT 1 FROM unnest(v_doctor_record.convenios_aceitos) AS convenio
      WHERE LOWER(TRIM(convenio)) = LOWER(TRIM(p_convenio))
