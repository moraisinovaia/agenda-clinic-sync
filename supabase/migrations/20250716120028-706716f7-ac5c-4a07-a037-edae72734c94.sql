-- Corrigir função get_current_user_profile que ainda falta search_path
CREATE OR REPLACE FUNCTION public.get_current_user_profile()
 RETURNS TABLE(id uuid, user_id uuid, nome text, email text, role text, ativo boolean, username text, created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE sql
 STABLE 
 SECURITY DEFINER
...
-- Corrigir função criar_agendamento_multiplo com search_path
