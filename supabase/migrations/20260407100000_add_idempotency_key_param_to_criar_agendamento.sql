-- P1: Idempotency hardening
-- Adiciona p_idempotency_key como parâmetro opcional à RPC criar_agendamento_atomico_externo.
-- A chave é gravada atomicamente no INSERT, eliminando o UPDATE best-effort que existia no repositório.

DROP FUNCTION IF EXISTS public.criar_agendamento_atomico_externo(
  uuid, text, date, text, text, text, uuid, uuid, date, time without time zone, text, text, boolean
);
