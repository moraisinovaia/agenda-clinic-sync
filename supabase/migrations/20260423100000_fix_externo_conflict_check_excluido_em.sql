-- Fix Bug #1: conflict check em criar_agendamento_atomico_externo não filtrava excluido_em IS NULL
--
-- Problema: agendamentos soft-deleted (cancelado_em IS NOT NULL ou excluido_em IS NOT NULL) com
-- status ainda em ('agendado','confirmado') bloqueavam novos slots no canal LLM Agent/WhatsApp,
-- enquanto o frontend (_atomico) já corrigia isso na migration 20260422200000.
-- Resultado: mesma data/hora aparecia disponível no portal mas era rejeitada via WhatsApp.
--
-- Fix: adiciona AND excluido_em IS NULL ao WHERE do SELECT de conflict check,
-- espelhando o comportamento corrigido na criar_agendamento_atomico (#9).
...
BEGIN
  ...
  -- Verificar conflitos (fix: AND excluido_em IS NULL — agendamentos soft-deleted não bloqueiam slot)
  ...
  -- Validar idade mínima (warning — não bloqueia, registra em observacoes)
  ...
  -- Validar idade máxima (warning — não bloqueia)
