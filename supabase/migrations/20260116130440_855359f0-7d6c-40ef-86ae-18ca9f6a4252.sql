-- Remover as versões antigas da função (com assinaturas diferentes)
DROP FUNCTION IF EXISTS public.criar_agendamento_atomico(
  text, date, text, text, text, uuid, uuid, date, time without time zone, text, text, uuid, boolean, uuid
);
...
-- Recriar função com assinatura única e correta
