...
-- 2. Remover e recriar a função get_current_user_profile com nova assinatura
DROP FUNCTION IF EXISTS public.get_current_user_profile();

-- 3. Criar função SECURITY DEFINER para verificar se usuário é admin (evita recursão)
...
-- 4. Recriar política para admins usando a função
CREATE POLICY "admins_can_see_all_profiles" ON profiles
FOR SELECT 
USING (public.is_admin_user());

-- 5. Recriar função get_current_user_profile com assinatura correta
