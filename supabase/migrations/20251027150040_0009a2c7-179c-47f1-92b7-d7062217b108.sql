-- SOLUÇÃO DEFINITIVA: Remover TODAS as versões e recriar com nome único
-- 1️⃣ Drop completo de TODAS as variações
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado() CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(date) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(date, date) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(date, date, uuid) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(date, date, uuid, text) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(timestamp with time zone) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(timestamp with time zone, timestamp with time zone) CASCADE;
DROP FUNCTION IF EXISTS buscar_agendamentos_otimizado(timestamp with time zone, timestamp with time zone, uuid) CASCADE;
...
-- 2️⃣ Recriar função com SETOF json (ZERO LIMIT - sem limitação PostgREST)
...
GRANT EXECUTE ON FUNCTION buscar_agendamentos_otimizado(timestamp with time zone, timestamp with time zone, uuid, text) TO anon;

-- 4️⃣ Comentário explicativo
COMMENT ON FUNCTION buscar_agendamentos_otimizado IS 'Retorna TODOS os agendamentos não excluídos usando SETOF json para bypass do limite PostgREST de 1000 registros. Atualizado em 2025-10-27.';