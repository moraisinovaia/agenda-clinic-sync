-- Fix crítico: paciente duplicado em criar_agendamento_atomico
--
-- Bug: a versão live da função buscava paciente com AND convenio = p_convenio e
--      AND data_nascimento = p_data_nascimento. Como convenio não faz parte do
--      idx_pacientes_unique_por_cliente (que é cliente_id + nome + data_nascimento),
--      qualquer agendamento do mesmo paciente com convênio diferente não achava o
--      registro existente → INSERT → unique constraint violation.
--      Secundariamente, NULL = NULL é FALSE no SQL, então pacientes sem data_nascimento
--      nunca eram encontrados, gerando duplicatas silenciosamente.
--
-- Fix: lookup por (cliente_id + nome_completo + data_nascimento) apenas, sem convenio.
--      NULL-safe via check explícito. INSERT com ON CONFLICT + fallback SELECT.
--      Idêntico ao padrão correto já presente em criar_agendamento_atomico_externo.
...
BEGIN
  ...
  --    Lookup por (cliente_id + nome + data_nascimento) — sem convenio.
  --    Convenio não identifica o paciente; muda de consulta para consulta.
  --    NULL-safe: data_nascimento IS NULL tratado explicitamente.
  ...
  IF v_paciente_id IS NULL THEN
    -- Tenta INSERT; se unique violation (data_nascimento IS NOT NULL), ON CONFLICT
    -- atualiza contato e retorna o id existente.
    ...
    -- ON CONFLICT não se aplica quando data_nascimento IS NULL (índice parcial).
    -- Nesse caso RETURNING volta NULL; faz SELECT + INSERT separado.
    IF v_paciente_id IS NULL THEN
      SELECT id INTO v_paciente_id
      FROM pacientes
      WHERE cliente_id = v_cliente_id
