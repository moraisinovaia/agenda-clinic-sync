-- Melhora resolução de serviço no trigger notificar_fila_webhook:
-- 1. Tenta match exato por atendimento_nome (elimina ambiguidade para procedimentos específicos)
-- 2. Fallback: heurística por período (consultas genéricas mapeadas em chaves temporais)
--
-- Dívida registrada: consultas genéricas (ex: "Consulta Gastroenterológica") não têm
-- chave exata em business_rules.servicos — fallback por período é a melhor heurística disponível.
-- Solução definitiva: adicionar campo servico_key em atendimentos ou fila_espera.
...
DECLARE
  ...
  payload          JSONB;
BEGIN
  -- 1. Dados básicos via joins (inclui atendimento_nome para match exato)
  SELECT
  ...
  IF v_br_config IS NOT NULL AND jsonb_typeof(v_br_config->'servicos') = 'object' THEN
    ...
    -- Tentativa 1: match exato por atendimento_nome
    IF v_br_config->'servicos' ? v_fila_record.atendimento_nome THEN
      v_servico_cfg := v_br_config->'servicos'->v_fila_record.atendimento_nome;
    ...
    -- Tentativa 2: fallback por período
    ...
    -- Fallback final: tipo_agendamento raiz
    IF v_tipo_raw IS NULL THEN
