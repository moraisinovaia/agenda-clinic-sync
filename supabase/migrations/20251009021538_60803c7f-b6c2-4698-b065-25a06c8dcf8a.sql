-- Corrigir função aprovar_usuario para atribuir cliente_id automaticamente
-- Todos os usuários devem ter cliente_id para que RLS funcione
...
BEGIN
  ...
  -- Buscar cliente IPADO (ou criar se não existir)
  ...
  -- Atualizar perfil: aprovar E atribuir cliente_id
  UPDATE public.profiles 
  SET 
    status = 'aprovado',
    aprovado_por = v_aprovador_profile_id,
    data_aprovacao = now(),
    cliente_id = v_cliente_ipado_id,  -- CRÍTICO: atribuir cliente_id
    updated_at = now()
  WHERE user_id = p_user_id;
...
-- Corrigir usuários já aprovados que não têm cliente_id
UPDATE public.profiles 
SET cliente_id = (SELECT id FROM public.clientes WHERE nome = 'IPADO' LIMIT 1)
WHERE status = 'aprovado' AND cliente_id IS NULL;