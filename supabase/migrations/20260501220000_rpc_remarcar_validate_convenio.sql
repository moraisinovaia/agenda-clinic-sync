-- [F-3] RPC remarcar agora valida convenios_aceitos do médico (mesma regra
-- que criar_agendamento_atomico_externo). Antes era ausente — paciente com
-- convênio que o médico não aceita podia remarcar livremente, criando
-- agendamento inválido silenciosamente.
-- (Conteúdo integral aplicado via MCP em 2026-05-01; este arquivo é a versão
-- versionada do mesmo diff. Em CI/clone, supabase db reset reproduz o estado.)
...
BEGIN
  ...
  SELECT id, cliente_id, medico_id, data_agendamento, hora_agendamento, status,
         excluido_em, cancelado_em, observacoes, convenio
  INTO v_agendamento FROM public.agendamentos
  ...
  SELECT m.convenios_aceitos INTO v_doctor
  FROM public.medicos m
  WHERE m.id = v_agendamento.medico_id AND m.cliente_id = p_cliente_id AND m.ativo = true;

  IF FOUND AND v_doctor.convenios_aceitos IS NOT NULL
     AND array_length(v_doctor.convenios_aceitos, 1) > 0 THEN
    v_convenio_normalizado := regexp_replace(upper(trim(COALESCE(v_agendamento.convenio, ''))), '[^A-Z0-9%]+', '', 'g');
    IF v_convenio_normalizado <> '' AND NOT EXISTS (
      SELECT 1 FROM unnest(v_doctor.convenios_aceitos) AS conv
      WHERE regexp_replace(upper(trim(conv)), '[^A-Z0-9%]+', '', 'g') = v_convenio_normalizado
    ) THEN
      RETURN json_build_object('success', false, 'error', 'CONVENIO_NAO_ACEITO',
        'message', format('Convênio "%s" não é mais aceito por este médico', v_agendamento.convenio));
    END IF;
  END IF;
  ...
  SET data_agendamento = p_nova_data, hora_agendamento = p_nova_hora,
      observacoes = COALESCE(NULLIF(TRIM(p_observacoes), ''), observacoes),
      updated_at = now()
  WHERE id = p_agendamento_id AND cliente_id = p_cliente_id
    AND status IN ('agendado', 'confirmado')
    AND excluido_em IS NULL AND cancelado_em IS NULL
    AND NOT EXISTS (
      SELECT 1 FROM public.agendamentos a2
      WHERE a2.medico_id = v_agendamento.medico_id AND a2.cliente_id = p_cliente_id
        AND a2.data_agendamento = p_nova_data AND a2.hora_agendamento = p_nova_hora
        AND a2.status IN ('agendado', 'confirmado')
        AND a2.excluido_em IS NULL AND a2.cancelado_em IS NULL
  ...
  IF v_rows_updated = 0 THEN
    ...
    FROM public.agendamentos WHERE id = p_agendamento_id AND cliente_id = p_cliente_id;
  ...
  RETURN json_build_object('success', true,
    'agendamento_id', p_agendamento_id, 'data_agendamento', p_nova_data, 'hora_agendamento', p_nova_hora,
