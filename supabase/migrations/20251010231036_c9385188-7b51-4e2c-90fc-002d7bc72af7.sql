-- Padronização de nomes em MAIÚSCULAS

-- Parte 1: Corrigir dados históricos
UPDATE public.pacientes 
SET nome_completo = UPPER(nome_completo)
WHERE nome_completo != UPPER(nome_completo);

-- Parte 2: Dropar e recriar função criar_agendamento_atomico
DROP FUNCTION IF EXISTS public.criar_agendamento_atomico(text,date,text,text,text,uuid,uuid,date,time without time zone,text,text,uuid,uuid,boolean);
...
BEGIN
  ...
  -- FORÇAR UPPER NO NOME DO PACIENTE
  ...
  IF v_paciente_id IS NULL THEN
    INSERT INTO public.pacientes (
      ...
      UPPER(TRIM(p_nome_completo)), p_data_nascimento, p_convenio, p_telefone, COALESCE(p_celular, '')
...
-- Parte 3: Dropar e recriar função criar_agendamento_multiplo
DROP FUNCTION IF EXISTS public.criar_agendamento_multiplo(text,date,text,text,text,uuid,uuid[],date,time without time zone,text,text,uuid);
