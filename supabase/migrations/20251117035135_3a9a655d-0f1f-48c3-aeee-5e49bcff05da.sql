-- Modificar função para agrupar automaticamente médicos "Marcelo" (excluindo MAPA)
...
DECLARE
  v_busca_medico TEXT;
BEGIN
  -- Lógica inteligente: se busca contém "Marcelo", simplifica para pegar todos os Marcelos
  v_busca_medico := CASE 
  ...
  RETURN COALESCE(
    (
      ...
      WHERE 
        m.nome ILIKE '%' || v_busca_medico || '%'
        AND a.data_agendamento = p_data
        AND a.status IN ('agendado', 'confirmado')
        AND a.excluido_em IS NULL
        AND a.cancelado_em IS NULL
        AND at.nome NOT ILIKE '%MAPA%'  -- Exclui todos os tipos de MAPA
    ),
    '[]'::jsonb
  );
END;
