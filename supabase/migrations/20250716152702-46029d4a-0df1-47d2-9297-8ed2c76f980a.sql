...
-- Atualizar configurações para produção (corrigir OTP)
UPDATE public.configuracoes_clinica 
SET valor = '300'  -- 5 minutos em segundos
WHERE chave = 'security_otp_expiry';

INSERT INTO public.configuracoes_clinica (chave, valor, categoria, ativo, dados_extras) 
VALUES 
  ('password_leak_protection', 'enabled', 'security', true, '{"description": "Proteção contra senhas vazadas habilitada"}'),
  ('production_ready', 'true', 'system', true, '{"description": "Sistema pronto para produção", "version": "1.0.0", "date": "2025-07-16"}')
