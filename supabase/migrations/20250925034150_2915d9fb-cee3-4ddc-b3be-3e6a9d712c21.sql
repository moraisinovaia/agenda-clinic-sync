-- FASE 1: Remover todas as versões conflitantes da função criar_agendamento_atomico
DROP FUNCTION IF EXISTS public.criar_agendamento_atomico(text, date, text, text, text, uuid, uuid, date, time, text, text, uuid);
DROP FUNCTION IF EXISTS public.criar_agendamento_atomico(text, date, text, text, text, uuid, uuid, date, time, text, text);
DROP FUNCTION IF EXISTS public.criar_agendamento_atomico(text, date, text, text, text, uuid, uuid, date, time, text);

-- FASE 2: Criar função unificada e completa com todos os parâmetros necessários (corrigindo sintaxe dos defaults)
...
BEGIN
  ...
  IF NOT v_is_editing OR p_forcar_conflito = false THEN
    SELECT COUNT(*)
    ...
    IF v_conflict_check > 0 AND p_forcar_conflito = false THEN
