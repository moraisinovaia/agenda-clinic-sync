...
-- Remove trigger/função anteriores (migration 20260422165255) antes de recriar
...
-- 1c) Libera slots cujo agendamento foi cancelado/excluído (corrige falsos 'ocupado')
UPDATE public.horarios_vazios hv
...
BEGIN
  ...
  -- DELETE ou virou inativo: liberar slot antigo
  ...
  -- UPDATE alterou data/hora/médico mantendo ativo: liberar slot antigo
  ...
  -- INSERT ou virou ativo: marcar slot como ocupado (upsert)
...
-- 4) Recriar os 3 triggers separados (INSERT, UPDATE, DELETE)
