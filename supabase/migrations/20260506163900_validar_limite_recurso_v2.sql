-- [Endogastro] validar_limite_recurso v2 — ciente do executor (JOANA).
--
-- Antes (v1): a função buscava distribuicao_recursos APENAS pelo p_medico_id
-- recebido. Quando a recepção agendava com medico=JOANA pra atendimento
-- "MAPA (DR DIEGO)", a função buscava distribuição pra JOANA (não tem) e
-- bloqueava com "MAPA não disponível para JOANA neste dia da semana".
--
-- Agora (v2): a função procura distribuição em TODOS os médicos vinculados
-- ao atendimento via medico_atendimento. Prioriza match direto com
-- p_medico_id; se não acha, usa qualquer cardio vinculado ao atendimento.
--
-- Pré-requisito: migration 20260506163800 vincula cardios aos atendimentos
-- com sufixo do médico (MAPA (DR DIEGO) ↔ Diego, etc).
--
-- Contagem de agendamentos também é v2: cobre os 2 modelos coexistentes
--   - Modelo 1: a.medico_id = cardio diretamente
--   - Modelo 2: a.medico_id = JOANA, mas atendimento vinculado ao cardio
-- pra evitar over-counting ou sub-counting.
...
BEGIN
  ...
  FROM atendimentos WHERE id = p_atendimento_id AND cliente_id = p_cliente_id;
  ...
  -- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  -- [v2] Procura distribuição em médicos vinculados ao atendimento
  -- via medico_atendimento, não apenas em p_medico_id.
  -- Prioriza match direto com p_medico_id se existir.
  -- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  WHERE d.recurso_id = v_recurso_id
    ...
    AND (
      ...
      OR d.medico_id IN (
        SELECT medico_id FROM medico_atendimento
  ...
  -- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  -- [v2] Conta agendamentos pra MESMO cardio da distribuição.
  -- Cobre 2 modelos:
  --   - Modelo 1: a.medico_id = cardiologista da distribuição (direto)
  --   - Modelo 2: a.medico_id = JOANA, mas atendimento vinculado ao cardio
  --     da distribuição via medico_atendimento.
  -- ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  WHERE a.data_agendamento = p_data_agendamento
    ...
    AND (
      a.medico_id = v_distribuicao.medico_id
      OR EXISTS (
        SELECT 1 FROM medico_atendimento ma
        WHERE ma.atendimento_id = a.atendimento_id
          AND ma.medico_id = v_distribuicao.medico_id
          AND ma.ativo = true
      )
    );
  ...
  IF v_agendados >= v_limite THEN
    RETURN json_build_object(
      ...
      'medico_nome', v_medico_distribuicao_nome,
      'data', p_data_agendamento,
