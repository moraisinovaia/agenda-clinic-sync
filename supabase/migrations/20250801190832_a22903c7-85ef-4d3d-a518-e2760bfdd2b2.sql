-- SOLUÇÃO DEFINITIVA: Corrigir conflitos de agendamento duplicado
-- Remover triggers primeiro, depois as funções

-- 1. Remover todos os triggers existentes relacionados
DROP TRIGGER IF EXISTS trigger_validar_agendamento_duplicado_medico ON public.agendamentos;
DROP TRIGGER IF EXISTS validar_agendamento_duplicado_medico_trigger ON public.agendamentos;
DROP TRIGGER IF EXISTS validar_agendamento_duplicado_paciente_trigger ON public.agendamentos;
DROP TRIGGER IF EXISTS validar_agendamento_multiplo_trigger ON public.agendamentos;

-- 2. Remover as funções
DROP FUNCTION IF EXISTS public.validar_agendamento_duplicado_medico();
DROP FUNCTION IF EXISTS public.validar_agendamento_duplicado_paciente();
DROP FUNCTION IF EXISTS public.validar_agendamento_multiplo();

-- 3. Recriar função de agendamento atômico com locks adequados
...
BEGIN
  ...
  -- CRITICAL: Verificar conflitos ANTES de qualquer outra operação
  SELECT COUNT(*)
...
-- 4. Criar trigger simplificado apenas para agendamentos múltiplos
CREATE OR REPLACE FUNCTION public.validar_agendamento_multiplo()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  -- Para agendamentos normais inseridos diretamente (bypass da função atômica)
  SELECT COUNT(*)
...
-- 5. Aplicar trigger apenas para INSERT direto (não UPDATE)
CREATE TRIGGER validar_agendamento_multiplo_trigger
  BEFORE INSERT ON public.agendamentos
  FOR EACH ROW
  EXECUTE FUNCTION public.validar_agendamento_multiplo();