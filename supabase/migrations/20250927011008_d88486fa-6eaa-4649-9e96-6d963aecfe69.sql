-- FASE 1: Correção Crítica de Security Search Path
-- Corrigindo 4 funções com search_path inseguro

-- 1. Corrigir função criar_agendamento_atomico
CREATE OR REPLACE FUNCTION public.criar_agendamento_atomico(p_nome_completo text, p_data_nascimento date, p_convenio text, p_telefone text, p_celular text, p_medico_id uuid, p_atendimento_id uuid, p_data_agendamento date, p_hora_agendamento time without time zone, p_observacoes text DEFAULT NULL::text, p_criado_por text DEFAULT 'recepcionista'::text, p_criado_por_user_id uuid DEFAULT NULL::uuid, p_agendamento_id_edicao uuid DEFAULT NULL::uuid, p_forcar_conflito boolean DEFAULT false)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_current_user_profile()
 RETURNS TABLE(id uuid, user_id uuid, nome text, email text, role text, ativo boolean, status character varying, username character varying, cliente_id uuid, created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE sql
...
-- 3. Corrigir função ensure_user_cliente_id
...
-- 4. Corrigir função get_user_cliente_id
...
INSERT INTO public.system_logs (
  ...
  now(), 'info', 
  'Correção crítica de segurança aplicada: Search Path das funções corrigido', 
  'SECURITY_FIX_SEARCH_PATH',
  jsonb_build_object(
    'functions_corrected', ARRAY['criar_agendamento_atomico', 'get_current_user_profile', 'ensure_user_cliente_id', 'get_user_cliente_id'],
    'security_issue', 'Function Search Path Mutable',
    'fix_applied', 'SET search_path TO public, pg_catalog'
  )
);