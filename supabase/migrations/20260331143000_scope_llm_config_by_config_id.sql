CREATE OR REPLACE FUNCTION public.load_llm_config_for_clinic(
...
BEGIN
  ...
  ELSE
    ...
    FROM llm_clinic_config lcc
    WHERE lcc.cliente_id = p_cliente_id
      AND lcc.ativo = true
    LIMIT 1;
  ...
  WHERE br.ativo = true
    AND (
      (v_config_id IS NOT NULL AND br.config_id = v_config_id)
      OR
      (v_config_id IS NULL AND br.cliente_id = COALESCE(v_cliente_id, p_cliente_id))
    );
  ...
  WHERE lm.ativo = true
    AND (
      (v_config_id IS NOT NULL AND lm.config_id = v_config_id)
      OR
      (v_config_id IS NULL AND lm.cliente_id = COALESCE(v_cliente_id, p_cliente_id))
    );
