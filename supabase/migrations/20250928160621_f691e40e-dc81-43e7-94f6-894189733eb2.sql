-- Atualizar função criar_agendamento_atomico para tornar data nascimento opcional para médicos Dr. Marcelo
...
BEGIN
  ...
  WHERE LOWER(nome_completo) = LOWER(p_nome_completo)
    AND (
      (p_data_nascimento IS NOT NULL AND data_nascimento = p_data_nascimento) OR 
      (p_data_nascimento IS NULL AND v_is_dr_marcelo)
    )
