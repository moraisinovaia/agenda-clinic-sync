CREATE OR REPLACE FUNCTION public.load_llm_config_for_clinic(p_cliente_id uuid DEFAULT NULL::uuid, p_config_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_config_id IS NOT NULL THEN
    ...
    -- Obter o cliente_id a partir do config_id
    SELECT lcc.cliente_id INTO v_cliente_id
  ...
  -- Carregar business_rules de TODOS os config_ids do mesmo cliente_id
  SELECT COALESCE(
    jsonb_object_agg(
      br.medico_id::text,
      jsonb_build_object(
  ...
  WHERE br.cliente_id = COALESCE(v_cliente_id, p_cliente_id) AND br.ativo = true;

  -- Carregar mensagens de TODOS os config_ids do mesmo cliente_id
  SELECT COALESCE(
    jsonb_object_agg(
  ...
  WHERE lm.cliente_id = COALESCE(v_cliente_id, p_cliente_id) AND lm.ativo = true;
