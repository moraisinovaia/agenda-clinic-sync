...
-- Note: Removed CONCURRENTLY from CREATE INDEX as it cannot run inside a transaction block
...
CREATE INDEX IF NOT EXISTS idx_agendamentos_cliente_medico_data 
ON public.agendamentos (cliente_id, medico_id, data_agendamento);

CREATE INDEX IF NOT EXISTS idx_pacientes_cliente_nome 
ON public.pacientes (cliente_id, nome_completo);

CREATE INDEX IF NOT EXISTS idx_profiles_cliente_status 
ON public.profiles (cliente_id, status, role);

CREATE INDEX IF NOT EXISTS idx_medicos_cliente_ativo 
ON public.medicos (cliente_id, ativo);

CREATE INDEX IF NOT EXISTS idx_atendimentos_cliente_ativo 
