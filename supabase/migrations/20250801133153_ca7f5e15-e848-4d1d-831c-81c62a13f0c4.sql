-- Atualizar a função criar_agendamento_atomico para não usar RAISE EXCEPTION em conflitos
-- Em vez disso, retornar JSON com success: false
...
BEGIN
  ...
  -- Verificar bloqueios de agenda - RETORNAR ERRO EM VEZ DE EXCEPTION
  SELECT COUNT(*)
  ...
  -- Verificar conflito de horário - RETORNAR ERRO EM VEZ DE EXCEPTION
  SELECT COUNT(*)
  ...
  -- Validar data/hora não é no passado - RETORNAR ERRO EM VEZ DE EXCEPTION (só para novos agendamentos)
...
EXCEPTION
  WHEN OTHERS THEN
    -- Em caso de erro inesperado, retornar JSON com erro
    RETURN json_build_object(
      'success', false,
