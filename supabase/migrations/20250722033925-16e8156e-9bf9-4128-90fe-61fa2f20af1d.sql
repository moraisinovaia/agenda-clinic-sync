-- DIA 1: VIEWS DE CONSULTA RÁPIDA PARA O DASHBOARD
...
SELECT 
  ...
  AVG(EXTRACT(EPOCH FROM l.timestamp - LAG(l.timestamp) OVER (PARTITION BY DATE(l.timestamp) ORDER BY l.timestamp))) as tempo_medio_segundos,
