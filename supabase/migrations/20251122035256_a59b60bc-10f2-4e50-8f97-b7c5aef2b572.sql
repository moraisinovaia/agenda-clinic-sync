...
BEGIN
  ...
  -- Validar convênio com comparação ROBUSTA (case-insensitive, ignorando espaços e pontuação)
  IF v_doctor_record.convenios_aceitos IS NOT NULL AND array_length(v_doctor_record.convenios_aceitos, 1) > 0 THEN
    -- Normalizar convênios removendo espaços/pontuação e deixando maiúsculo
    SELECT EXISTS (
      SELECT 1
      FROM unnest(v_doctor_record.convenios_aceitos) AS convenio
      WHERE regexp_replace(upper(convenio), '[^A-Z0-9%]+', '', 'g') =
            regexp_replace(upper(p_convenio), '[^A-Z0-9%]+', '', 'g')
